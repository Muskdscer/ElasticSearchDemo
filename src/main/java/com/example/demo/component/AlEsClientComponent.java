...
public class AlEsClientComponent {
    ...
    public PageEs searchAfter(int pageSize, String index,
        ...
        try {
            ...
            SearchHits searchHits = searchResponse.getHits();
            SearchHit[] result = searchHits.getHits();
            hitList.addAll(Arrays.asList(result));

            while (result.length > 0) {
                SearchHit last = result[result.length - 1];
...
//                }
                ...
                result = searchHits.getHits();
                hitList.addAll(Arrays.asList(result));
            }


            for (SearchHit searchHit : hitList){
                System.out.println("查询结果为： " + searchHit.getSourceAsString());
                jsonArray.add(JSON.parse(searchHit.getSourceAsString()));
            }
            PageEs pageEs = new PageEs();
            pageEs.setPageSize(pageSize);
            pageEs.setResult(jsonArray.toJSONString());
            pageEs.setTotal(Long.valueOf(hitList.size() + ""));
            System.out.println("共有：" + hitList.size() + "条记录.");
            return pageEs;
        } catch (Exception e) {
            e.printStackTrace();
        }
